...
# ─────────────────────────────────────────────────────────────────────────────
# SIMULATION WARNING — READ BEFORE USING
# ─────────────────────────────────────────────────────────────────────────────
# This class generates *synthetic* performance metrics for UI demos and testing.
# All output dicts carry "source": "simulated" so callers can distinguish.
#
# For real measurements (required for resume / publication):
#   1. BN Fold + ReLU6 surgery:
#      pip install aimet-torch
#      python q1_compression_suite/compression/real_onnx_benchmark.py
#
#   2. Snapdragon X Elite on-device latency:
#      pip install qai-hub && qai-hub configure --api-token <FREE_TOKEN>
#      python q1_compression_suite/deployment/run_aihub_profile.py --model mobilenet_v2
#
#   3. INT8 accuracy drop:
#      Cited from Nagel et al., "Up or Down? Adaptive Rounding for Post-Training
#      Quantization" (AdaRound), ICML 2020, Table 2.
#      MobileNetV2 W8A8: <0.5% top-1 drop on ImageNet-1k.
# ─────────────────────────────────────────────────────────────────────────────

_SIMULATION_WARNING = (
    "SIMULATED value — generated by CompressionSimulator, not real AIMET or "
    "real hardware. See real_onnx_benchmark.py and run_aihub_profile.py for "
    "actual measurements."
)
...
class CompressionSimulator:
    """
    Fallback simulation of the AIMET PTQ pipeline for UI demos / unit tests.

    All returned dicts include ``"source": "simulated"`` so dashboards and
    reports can clearly label which numbers are real vs synthetic.

    Do NOT use these numbers in a resume, paper, or benchmark claim.
    """
    ...
    def simulate_stage(self, model_name: str, stage_name: str, ood_calibration: bool = False) -> Dict[str, Any]:
        ...
        return {
            ...
            "status": "passed",
            "source": "simulated",
            "simulation_warning": _SIMULATION_WARNING
        }
